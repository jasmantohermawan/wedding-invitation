...
export const wishas = () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            await commentsService.addComments(comments);

            lengthComments = response.comments.length;

            peopleComments.textContent = `${++response.comments.length} Orang telah mengucapkan`;
            containerComments.insertAdjacentHTML('afterbegin', listItemComments(comments));
        } catch (error) {
            return `Error : ${error.message}`;
        } finally {
